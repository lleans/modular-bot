...
class Reaction:
    __BAD_EXPRESSION = ["Katanya kalo pura pura gila\ngila beneran", 
            "In the name of goddess Aqua\nI doakan U cepet mampus", 
            "Yg Patah Tumbuh\nYg Hilang Otak Kau", "Kamu Kok Betah Banget Jadi Goblok", 
    ...
    __RESPONSE = {
        'bad_words': ["Astaugfirullah...", "Ngak boleh gitu", "Goblok", "Yang sopan dong", "Utekke ng silet", "Bocah goblok", "Lambemu su"],
            'stress': ["blok", "Halo Pak Nasikind, ada yang perlu di ruqyah", "Stress", "Goblok", "Kalo stress jangan disini", "Ingatlah nak 2D itu ngak nyata", "3D > 2D"],
            'holy_word': ["Masyallah", "Subhanallah", "Stay halal brader", "Alhamdulillah", "Allah akbar"]
    }

    @classmethod
    def message_generator(cls, message: str) -> Union[str, Embed]:
        ...
        for type, item in cls.__REACT_TO.items():
            if item.search(clean):
                ...
                if not sendtype:
                    rand_express: str = choice(cls.__BAD_EXPRESSION) if type == "bad_words" or type == "stress" else choice(cls.__GOOD_EXPRESSION)
                    e: Embed = Embed(title=exprss, description=rand_express, color=ModularUtil.convert_color(choice(ModularBotConst.COLOR['random_array'])))
                    return e
                return exprss