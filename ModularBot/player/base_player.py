	...
	def is_user_join_checker(cls):
		async def decorator(interaction: Interaction) -> bool:
			...
			if not user_voice_state:
				...
				guild_voice_client
				and guild_voice_client.channel != user_voice_state.channel
			):
				await ModularUtil.send_response(
					interaction,
...
class TrackPlayerBase:
	...
	async def _custom_wavelink_searcher(
		...
		if was_youtube:
			tracks = await Playable.search(
				...
				if track_type is not TrackType.YOUTUBE_MUSIC
				else TrackSource.YouTubeMusic,
			)
	...
	async def _play_response(
		...
		*,
		...
		if is_playlist:
			embed.description = (
				f"✅ Queued {'(on front)' if is_put_front else ''} - {len(tracks)} \
             tracks from ** [{tracks.name}]({tracks.url})**"
			)

		elif is_queued:
			embed.description = f"✅ Queued {
				'(on front)' if is_put_front else ''
