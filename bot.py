	...
	async def __ramadhan_checker(self) -> None:
		...
		if self._praytimes:
			...
			ramadhan_start: datetime = datetime.strptime(
				f"{self._praytimes['ramadhan']['start']} {time.strftime('%Y')}",
				"%B %d %Y",
			)
			ramadhan_end: datetime = datetime.strptime(
				f"{self._praytimes['ramadhan']['end']} {time.strftime('%Y')}",
				"%B %d %Y",
			)
		...
		if self._praytimes:
			...
			if self._is_ramadhan:
				...
				if is_praytime is not None:
					...
					self._praytime_message = await self._praytime_channel.send(
						embed=is_praytime
					)
	...
	async def _lockdown_channel(self) -> None:
		...
		async def _do_the_lockdown(view_channel: bool):
			for channel in self._guild.text_channels:
				...
				if (
					channel.is_nsfw()
					and channel.id != GuildChannel.BINCANG_HARAM_CHANNEL
					and overwrites.view_channel != view_channel
				):
		...
		async def a() -> None:
			await self.change_presence(
				activity=Activity(
					...
					name=f"{the_musketter_count} Hamba Allah",
				)
			)
		...
		async def b() -> None:
			await self.change_presence(
				activity=Activity(
					...
					name=f"bareng {the_musketter_count} Member",
				)
			)
		...
		async def c() -> None:
			await self.change_presence(
				activity=Activity(
					...
					name=f"@{ModularBotConst.BOT_NAME.replace(' ', '')}",
				)
			)
		...
		async def d() -> None:
			await self.change_presence(
				activity=Activity(
					...
					name=f"{ModularBotConst.SERVER_NAME.replace(' ', '')}",
				)
			)
...
class ModularBotBase(commands.Bot):
	...
	async def _help_embed(self) -> Embed:
		desc: str = (
			f"Here's a few feature that's available on {ModularBotConst.SERVER_NAME}."
		)
		embed: Embed = Embed(
			title=f"Menu {ModularBotConst.SERVER_NAME}",
			description=desc,
			color=ModularUtil.convert_color(choice(ModularBotConst.Color.RANDOM)),
		)
		...
		for command in await self.tree.fetch_commands():
			if command.description:
				...
				embed.add_field(
					name=f"**{command.name}**",
					value="Access it through Apps Command",
					inline=True,
				)
...
class ModularBotClient(ModularBotBase, ModularBotTask):
	...
	async def on_member_join(self, member: Member) -> None:
		...
		welcome_banner: BytesIO = await ModularUtil.banner_creator(
			str(member.display_name), member.display_avatar.url
		)
	...
	async def on_member_remove(self, member: Member) -> None:
		leave_banner: BytesIO = await ModularUtil.banner_creator(
			str(member.display_name), member.display_avatar.url, is_welcome=False
		)
	...
	async def on_message(self, message: Message) -> None:
		if message.author == self.user:
			...
			self.user.mentioned_in(message=message)
			and self.user.mention in message.content
		):
...
def main() -> None:
	...
	async def _help(interaction: Interaction) -> None:
		await ModularUtil.send_response(
			interaction, embed=await bot._help_embed(), ephemeral=True
		)
	...
	async def _ping(interaction: Interaction) -> None:
		...
		if where:
			...
			}ms, bot server location {where} :earth_americas:"
